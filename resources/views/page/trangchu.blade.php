...
// khởi tạo kích thước của ô vuông
$size = 2;

// khởi tạo một mảng 2 chiều để chứa các chữ cái A-Z
$letters = array();
for ($i = 0; $i < $size; $i++) {
  $letters[$i] = array();
}

// tạo chuỗi các chữ cái A-Z
$alphabet = range('A', 'Z');

// khởi tạo biến đếm và vòng lặp while để đổ các chữ cái vào mảng 2 chiều
$count = 0;
while ($count < $size * $size) {
  $letters[$count % $size][floor($count / $size)] = $alphabet[$count % 26];
  $count++;
}

// in ra mảng 2 chiều để tạo ra ô vuông
for ($i = 0; $i < $size; $i++) {
  for ($j = 0; $j < $size; $j++) {
    echo $letters[$i][$j] . ' ';
  }
